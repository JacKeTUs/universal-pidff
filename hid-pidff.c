...
static int pidff_erase_effect(struct input_dev *dev, int effect_id)
{
	...
	if (pidff->effect_count > 0)
		hid_dbg(pidff->hid, "current effect count: %d",
			--pidff->effect_count);
	return 0;
}
...
{
	struct pidff_device *pidff = dev->ff->private;
	int type_id = pidff_get_effect_type_id(pidff, effect);
	...
	/* Unfortunate naming, but the first one is report type name */
	PIDFF_SET_REPORT_IF_NEEDED(effect, effect, old);
	switch (effect->type) {
	case FF_CONSTANT:
		PIDFF_SET_REPORT_IF_NEEDED(constant, effect, old);
		PIDFF_SET_ENVELOPE_IF_NEEDED(constant, effect, old);
		break;

	case FF_PERIODIC:
		PIDFF_SET_REPORT_IF_NEEDED(periodic, effect, old);
		PIDFF_SET_ENVELOPE_IF_NEEDED(periodic, effect, old);
		break;

	case FF_RAMP:
		PIDFF_SET_REPORT_IF_NEEDED(ramp, effect, old);
		PIDFF_SET_ENVELOPE_IF_NEEDED(ramp, effect, old);
		break;
	...
	case FF_INERTIA:
	case FF_FRICTION:
		PIDFF_SET_REPORT_IF_NEEDED(condition, effect, old);
		break;
	}
